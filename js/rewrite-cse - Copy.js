...
(function() {
    ...
    // fix-me: Seems this is done twice for each link. Doesn't it need a closure???
    ...
    var new_startRewriterWithNodeCheck = (function() {
        var timer; // persisted in closure!
        return function() {
            window.clearTimeout(timer);
            timer = setTimeout(function(){
                var lnks = document.querySelector(
                    "#___gcse_0 .gsc-wrapper .gsc-result div.gs-title a.gs-title");
                if (lnks) {
                    lnks.dataset.mycheck = "started";
                }
            }, 500);
    ...
    function rewriteLinks() {
        ...
        var first = [];
        var rq = [];
        console.log("lnks.length", lnks.length);
        for (var i=0; i<lnks.length; i++) {
            ...
            var currentTitle = lnks[i].textContent;
    ...
    function requestTitleUpdate(node) {
        ...
        // fix-me: timing problem, we do not have the json yet...
        var oldJson = cachedJsons[origURL];
        if (oldJson) {
